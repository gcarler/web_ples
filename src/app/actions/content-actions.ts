...
export async function getHeroStatements(): Promise<HeroStatement[]> {
  const defaultStatements = [
      {
        title: "Datos, ingenier?a y prop?sito \n para el {{word}}",
        description: "De la idea a la acci?n: acompa?amos a gobiernos y empresas a generar impacto real.",
        ctaText: "Empieza hoy",
        ctaLink: "/forms",
        ctaIconName: "Send",
        ctaVariant: 'accent',
...
export async function getCoreValues(): Promise<CoreValue[]> {
    const defaultValues = [
        { id: 'colaboracion', name: 'COLABORACI?N', iconName: 'UsersIcon', explanation: "La colaboraci?n es la esencia de nuestro accionar. Fomentamos la sinergia entre equipos multidisciplinarios y promovemos alianzas estrat?gicas para co-crear soluciones integrales que superan las expectativas y generan un impacto duradero.", order: 0 },
        { id: 'innovacion', name: 'INNOVACI?N', iconName: 'Lightbulb', explanation: "Como motor de nuestro progreso, la innovaci?n nos impulsa a desafiar el status quo y a explorar constantemente nuevas tecnolog?as y metodolog?as. Convertimos ideas audaces en soluciones pr?cticas que aportan un valor tangible y sostenible a nuestros clientes.", order: 1 },
        { id: 'integridad', name: 'INTEGRIDAD', iconName: 'Shield', explanation: "Actuamos con honestidad, transparencia y ?tica profesional en cada interacci?n. La integridad es el pilar de la confianza que construimos con nuestros clientes, socios y la comunidad, garantizando que nuestras acciones siempre est?n alineadas con nuestros principios.", order: 2 },
    ];
    if (!adminDb) {
...
export async function getPillars(): Promise<Pillar[]> {
    const defaultPillars = [
        { id: 'ciencia', title: 'Ciencia', description: 'El rigor metodol?gico como pilar de la confianza y la efectividad.', link: '/porque-somos-ciencia', iconName: 'FlaskConical', order: 0 },
        { id: 'tecnologia', title: 'Tecnolog?a', description: 'Herramientas de vanguardia como catalizadores de la eficiencia y la escala.', link: '/porque-somos-tecnologia', iconName: 'Cpu', order: 1 },
        { id: 'innovacion', title: 'Innovaci?n', description: 'La creatividad y el pensamiento disruptivo para generar valor sostenible.', link: '/porque-somos-innovacion', iconName: 'Lightbulb', order: 2 },
    ];
    if (!adminDb) {
