...
export default function AdminLayout({ children }: PropsWithChildren) {
  ...
  /*
  // NOTE: This client-side redirect has been temporarily disabled.
  // It is the reason you were being redirected to the /login page.
  // The application correctly determines that no user is logged in (because Firebase is not configured)
  // and redirects as a security measure. To re-enable it, uncomment this block after
  // configuring your Firebase credentials in .env.local.
  useEffect(() => {
    if (!loading && !user) {
      router.push('/login');
    }
  ...
  */
  ...
  // Because the redirect is disabled, we show a skeleton view if the user is not logged in.
  // This allows you to see the admin UI layout without being logged in.
  if (!user) {
    return (
     <SidebarProvider>
        <Sidebar>
            ...
            <SidebarContent>
             ...
             <SidebarFooter>
                <SidebarMenu>
                     <SidebarMenuItem>
                         <SidebarMenuButton className="text-destructive hover:bg-destructive/10 focus:bg-destructive/10">
                             <LogOut />
